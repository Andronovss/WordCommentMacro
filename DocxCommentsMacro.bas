...
'=========================
    ...
    Title = "Экспорт комментариев в новый докумен"
    Set oDoc = ActiveDocument
    nCount = ActiveDocument.Comments.Count
    
    If nCount = 0 Then
        MsgBox "Докумен не содержит комментариев", vbOKOnly, Title
        GoTo ExitHere
    Else
    ...
    'Создание новго документа на основе шаблона.dotm.
    ...
    'В заголовок таблицы помещаем следующий контен:
    'Из какого файла делается экспорт
    'Кто экспортирует комментарии
    'Дата экспорта комментариев.
    oNewDoc.Sections(1).Headers(wdHeaderFooterPrimary).Range.Text = _
        "Èñõîäíûé ôàéë: " & oDoc.FullName & vbCr & _
        "Àâòîð: " & Application.UserName & vbCr & _
        "Äàòà ñîçäàíèÿ: " & Format(Date, "MMMM d, yyyy")
    ...
    For n = 1 To nCount
        With oTable.Rows(n + 1)
            ...
            'Текст, которым был помечен комментарий
